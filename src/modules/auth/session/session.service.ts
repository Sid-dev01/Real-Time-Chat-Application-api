...
export class SessionService {
  ...
  async createSession(data: CreateSessionData): Promise<TokenPair> {
    ...
    const [refreshToken, expiresAt] = await Promise.all([
      this.tokenHashService.hash(authTokens.refreshToken),
  ...
  async rotateRefreshToken(userId: string, sessionId: string): Promise<TokenPair> {
      
    const authTokens = await this.tokenService.generateAuthTokens(userId, sessionId);


    const [refreshTokenHash, expiresAt] = await Promise.all([
      this.tokenHashService.hash(authTokens.refreshToken),
      Promise.resolve(this.tokenService.calculateRefreshTokenExpiration()),
    ]);
